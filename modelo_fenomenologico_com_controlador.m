function [A1, A2, h1, h2] = modelo_fenomenologico_com_controlador(Heq_1, Heq_2, dt, T,  h0_1, h0_2, resumo_c)
...
% Obtem faixa de kps, kis e kds
[pack_ganhos] = ganhos(resumo_c);

for i=1:N
    
    %% Cálculo de u(t)
    if i ~= 1 % Vamos ignorar a primeira amostra
        % Tanque 1
        erro1 = Heq_1 - h1(i);
        [qin1(i), ultima_integral_1, ultimo_erro_1] = pi_tanque(h1(i), erro1, ultima_integral_1, ultimo_erro_1, dt, 1, pack_ganhos);
        % Tanque 2
        erro2 = Heq_2 - h2(i);
        [qin2(i), ultima_integral_2, ultimo_erro_2] = pi_tanque(h2(i), erro2, ultima_integral_2, ultimo_erro_2, dt, 2, pack_ganhos);
    end
    ...
    %% Cálculo da equação diferencial
    qout1 = k1 * sqrt(h1(i));
    qout2 = k2 * sqrt(h2(i));
